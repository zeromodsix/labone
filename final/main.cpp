...
bool cmpStrLen(std::string str1, std::string str2);
...
int main()
{
  ...
  std::cout<<"Enter the directory in which files are to be indexed: ";
  std::string dName;
  std::cin>>dName;
  if(dName == "")
    dName = "files";
  dName = "ls " + dName + "/*.txt>/tmp/files.txt";
  system(dName.c_str());
  ...
  do{
	...
	unsigned int width = (*std::max_element(fNames, fNames + numFiles, cmpStrLen)).length() + 2;
	for(int i = 0, flag =1; i<numFiles && flag; i++)
	  {
	    if(flag = templist[counter[i]].ret_count()) /* the = is intentional */ 
	      std::cout<<std::setw(width)<<fNames[counter[i]]<<": "<<std::setw(8)<<templist[counter[i]].ret_count()<<std::endl; 
	  }
	delete[] counter;
      }
